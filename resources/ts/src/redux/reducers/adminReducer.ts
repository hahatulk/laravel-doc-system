...
export default function admin(state = initial_state, action: any) {
    ...
    switch (action.type) {
        case 'STUDENTS_LIST_GET':
            ...
            //     studentsList = studentsList.map((el: any, i) => {
            //         // el.birthday = getMoscowPlainDateTime(el.birthday)
            //         // el.formaObuch = el.formaObuch === 0 ? 'бюджетная' : 'платная'
            //         return el
            //     })
            // }
        ...
        case 'ARCHIVED_STUDENTS_LIST_GET':
            let archivedStudentsList: any[] = p.data.data.students?.length
                ? p.data.data.students
                : []

            // if (archivedStudentsList?.length) {
            //     archivedStudentsList = archivedStudentsList.map((el: any, i) => {
            //         // el.birthday = getMoscowPlainDateTime(el.birthday)
            //         el.formaObuch = el.formaObuch === 0 ? 'бюджетная' : 'платная'
            //         return el
            //     })
            ...
            return {
                ...
                archivedStudentsList: {
                    ...
                    count: p.data.data.studentsCount ? p.data.data.studentsCount : 0
                }
            }
        ...
        case 'ORDERS_LIST_GET':
            let ordersList: any[] = p.data.data.orders?.length
                ? p.data.data.orders
                : []
            return {
                ...state,
                ordersList: {
                    loading: false,
                    list: ordersList,
                    count: p.data.data.ordersCount ? p.data.data.ordersCount : 0
                }
            }
        ...
        case 'ARCHIVED_ORDERS_LIST_GET':
            let archivedOrdersList: any[] = p.data.data.orders?.length
                ? p.data.data.orders
                : []
            return {
                ...state,
        ...
        case 'GROUPS_LIST_GET':
            let groupsList: any[] = p.data.data.groups?.length
                ? p.data.data.groups
                : []
            ...
            return {
                ...
                groupsList: {
                    loading: false,
                    list: groupsList,
                    count: p.data.data.groupsCount ? p.data.data.groupsCount : 0
                }
            }
        ...
        case 'PRIKAZ_LIST_GET':
            let prikazList: any[] = p.data.data.prikaz?.length
                ? p.data.data.prikaz
                : []
            ...
            return {
                ...
                prikazList: {
                    loading: false,
                    list: prikazList,
                    count: p.data.data.prikazCount ? p.data.data.prikazCount : 0
                }
            }
        ...
        case 'MODERATORS_LIST_GET':
            let moderatorsList: any[] = p.data.data.moderators?.length
                ? p.data.data.moderators
                : []
            ...
            return {
                ...
                moderatorsList: {
                    loading: false,
                    list: moderatorsList,
                    count: p.data.data.moderatorsCount ? p.data.data.moderatorsCount : 0
                }
            }
